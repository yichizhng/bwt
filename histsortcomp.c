...
// This used to be called for depth=0, but for multithreading purposes it isn't
...
// A struct for pthreads, allowing us to pass arguments to the new threads
struct hh_args {
  const char *base;
  int len;
  int *arr;
...
// Wrapper function for histhelper for pthreads
