...
// TO THINK ABOUT: Why does getchar() work correctly when it relies
// on shifts? It works on signed char, not unsigned (maybe just
// because it's a char type?)
// TODO: Replace char with unsigned char everywhere probably
unsigned int *suff_arr(char *seq, unsigned int len) {
	// seq is assumed to be given in *uncompressed* form and be
	// null-terminated (having an internal zero byte is fine)
	// TODO: I believe replacing s[*] by getbase(s, *) may fix this
	// (We can also probably restrict the bukkits to 4, since we're
	// not *really* working with chars :))
	// We may also have to set a virtual sentinel at the end (i.e.
	// append another A)
	// Testing, ahoy!
